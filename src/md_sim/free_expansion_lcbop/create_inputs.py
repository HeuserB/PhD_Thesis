...
f = open("in.free_expansion_lcbop_template",'r')
in_txt = f.read()
f.close()

f = open("lammps.SLURM_free_expansion_lcbop",'r')
slurm_txt = f.read()
f.close()
...
def input_temp(in_txt, slurm_txt, temp, grid_constant, n_cells, t_init=17000.0,seed=123456789):
    ...
    dumpfile = '/data/bh326/LAMMPS/free_expansion_lcbop/grid_' +str(grid) +'/' + str(temp) + 'K_' +str(n_cells) +'x' + str(n_cells) +'x' + str(n_cells)
    logfile = '/data/bh326/LAMMPS/free_expansion_lcbop/grid_' +str(grid) +'/' + str(temp) + 'K_' +str(n_cells) +'x' + str(n_cells) +'x' + str(n_cells) +'.log'

    in_txt = re.sub(r'\$dumpfile',dumpfile,in_txt)
    in_txt = re.sub(r'\$logfile',logfile,in_txt)

    outfile = 'inputs_free_expansion/in.free_expansion_lcbop_' + str(temp) + 'K_' +str(n_cells) +'x' + str(n_cells) +'x' + str(n_cells) + 'grid_' + str(grid)
    ...
    outfile_slurm  = 'inputs_free_expansion/lammps.free_expansion_lcbop_' + str(temp) +'K_' +str(n_cells) +'x' + str(n_cells) +'x' + str(n_cells) + 'grid_' + str(grid)
